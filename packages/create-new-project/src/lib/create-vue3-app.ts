...
export default async function createVue3App(
  projectName: string,
  templatePath: string,
  targetPath: string
) {
  ...
  const env = await readFile(resolve(targetPath, '.env'), 'utf-8');
  await writeFile(
    resolve(targetPath, '.env'),
    env.replace(/\{\{PROJECT_NAME\}\}/g, projectName),
    'utf-8'
  );
  console.log(chalk.green('📦 生成项目模版成功'));
  const workflow = await readFile(resolve(templatePath, '../.ci/app.template.yml'), 'utf-8');
  await writeFile(
