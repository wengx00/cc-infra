...
export default async function createNodejsPackage(
  projectName: string,
  templatePath: string,
  targetPath: string
) {
  ...
  const workflow = await readFile(
    resolve(templatePath, '../.ci/packages-nodejs.template.yml'),
    'utf-8'
  );
  await writeFile(
    resolve(process.cwd(), `.github/workflows/packages__${projectName}.yml`),
    workflow.replace(/\{\{PROJECT_NAME\}\}/g, projectName),
    'utf-8'
  );
  console.log(
    chalk.green('🎉 生成流水线成功 -> ', `./.github/workflows/packages__${projectName}.yml`)
  );
}
